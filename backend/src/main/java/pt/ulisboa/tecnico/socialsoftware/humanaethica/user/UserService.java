...
public class UserService {
    ...
    public VolunteerProfileDto createVolunteerProfile(Integer userId, VolunteerProfileDto volunteerProfileDto) {
        ...
        if (volunteerProfileDto == null) {
            throw new HEException(VOLUNTEERPROFILE_INVALID_ARGUMENT);
        }

        Volunteer volunteer = (Volunteer) userRepository.findById(userId)
            .orElseThrow(() -> new HEException(USER_NOT_FOUND, userId));

        List<Participation> selectedParticipations = new ArrayList<>();
        List<Integer> participationIds = volunteerProfileDto.getSelectedParticipationsIds() != null 
                ? volunteerProfileDto.getSelectedParticipationsIds() 
                : Collections.emptyList();

        for (Integer id : participationIds) {
            Participation participation = participationRepository.findById(id)
                .orElseThrow(() -> new HEException(PARTICIPATION_NOT_FOUND, id));
            selectedParticipations.add(participation);
        }
    ...
    public VolunteerProfileDto getVolunteerProfile(Integer userId) {
        ...
        VolunteerProfile volunteerProfile = volunteerProfileRepository.findById(userId).orElseThrow(() -> new HEException(VOLUNTEERPROFILE_NOT_FOUND));
