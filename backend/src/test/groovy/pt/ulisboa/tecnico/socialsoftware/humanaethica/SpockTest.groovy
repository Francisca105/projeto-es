...
class SpockTest extends Specification {
    ...
    public static final String VOLUNTEERPROFILE_SHORT_BIO = "Valid shortBio with more than 10 characters."

    @Autowired
    VolunteerProfileRepository volunteerProfileRepository
    
    def createVolunteerProfileDto() {
        def volunteer = createVolunteer(USER_1_NAME, USER_1_PASSWORD, USER_1_EMAIL, AuthUser.Type.NORMAL, User.State.APPROVED)

        def institution1 = createInstitution(INSTITUTION_1_NAME, INSTITUTION_1_EMAIL, INSTITUTION_1_NIF)
        ...
        activityRepository.save(activity1) // Ensure activity is persisted

        def enrollment1 = new Enrollment(activity1, volunteer, new EnrollmentDto(motivation: "Valid motivation with more than 10 caracters."))
        activity1.addEnrollment(enrollment1)
        enrollmentRepository.save(enrollment1) // Save enrollment

        // Create participations, add to volunteer, and save
        def participation1 = new Participation(volunteer: volunteer, activity: activity1)
        participation1.setVolunteerRating(5)
        volunteer.addParticipation(participation1)
        participationRepository.save(participation1)

        def participation2 = new Participation(volunteer: volunteer, activity: activity1)
        participation2.setVolunteerRating(4)
        volunteer.addParticipation(participation2)
        participationRepository.save(participation2)

        // Ensure volunteer's participations are updated and saved
        ...
        volunteerProfileDto.setSelectedParticipationsIds([participation1.id, participation2.id]) // Use IDs
