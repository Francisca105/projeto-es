...
class CreateVolunteerProfileWebServiceIT extends SpockTest {
    ...
    def "volunteer create VolunteerProfile"() {
        given:
        demoVolunteerLogin()

        when:
        def response_profile = webClient.post()
        ...
        then:
        volunteerProfileRepository.count() == 1
        response_profile.shortBio == VOLUNTEERPROFILE_SHORT_BIO
        response_profile.volunteerId == volunteerProfileDto.getVolunteerId()
        response_profile.selectedParticipationsIds == volunteerProfileDto.getSelectedParticipationsIds()

        cleanup:
        deleteAll()
    }

    def "volunteer create VolunteerProfile invalid"() {
        given:
        demoVolunteerLogin()

        when:
        def response_profile = webClient.post()
            .uri("/volunteers/abc/profile")
            .headers(httpHeaders -> httpHeaders.putAll(headers))
            .bodyValue(volunteerProfileDto)
            .retrieve()
            .bodyToMono(VolunteerProfileDto.class)
    ...
    def "volunteer create VolunteerProfile with an error"() {
        given:
        demoVolunteerLogin()
        volunteerProfileDto.setShortBio(null) // invalid ShortBio
    ...
    def "admin cannot create VolunteerProfile"() {
        given:
        demoAdminLogin()

        when:
        def response_profile = webClient.post()
            .uri("/volunteers/1/profile")
            .headers(httpHeaders -> httpHeaders.putAll(headers))
            .bodyValue(volunteerProfileDto)
            .retrieve()
            .bodyToMono(VolunteerProfileDto.class)
    ...
    def "member cannot create VolunteerProfile"() {
        given:
        demoMemberLogin()
